 ...
 */
 drop table if exists publ.user_emails cascade;
create table publ.user_emails (
  id uuid primary key default gen_random_uuid(),
  user_id uuid not null default publ.current_user_id() references publ.users on delete cascade,
...
alter table publ.user_emails enable row level security;

create policy select_all on publ.user_emails for select to :DATABASE_VISITOR using (true);
...
create unique index uniq_user_emails_verified_email on publ.user_emails(email) where (is_verified is true);
-- Only one primary email per user.
create unique index uniq_user_emails_primary_email on publ.user_emails (user_id) where (is_primary is true);
-- Allow efficient retrieval of all the emails owned by a particular user.
create index idx_user_emails_user on publ.user_emails (user_id);
-- For the user settings page sorting
create index idx_user_emails_primary on publ.user_emails (is_primary, user_id);

-- Keep created_at and updated_at up to date.
create trigger _100_timestamps
  before insert or update on publ.user_emails
  for each row
  execute procedure priv.tg__timestamps();

-- You can't verify an email address that someone else has already verified. (Email is taken.)
create or replace function publ.tg_user_emails__forbid_if_verified() returns trigger as $$
begin
  if exists(select 1 from publ.user_emails where email = NEW.email and is_verified is true) then
    raise exception 'An account using that email address has already been created.' using errcode='EMTKN';
  end if;
  return NEW;
end;
$$ language plpgsql volatile security definer set search_path to pg_catalog, public, pg_temp;
-- this trigger checks if the email has already been verified and if yes, it displays an error message, if not, it allows the email to be insert
create trigger _200_forbid_existing_email 
before insert on publ.user_emails 
for each row 
execute procedure publ.tg_user_emails__forbid_if_verified();
...
create trigger _900_send_verification_email
  ...
  execute procedure priv.tg__add_job('verify_email_address');
-- we do not pass queue name as second argument.


comment on table publ.user_emails is
  E'Information about a user''s email address.';
comment on column publ.user_emails.email is
  E'The users email address, in `a@b.c` format.';
comment on column publ.user_emails.is_verified is
...
create policy select_own on publ.user_emails for select using (user_id = publ.current_user_id());
create policy insert_own on publ.user_emails for insert with check (user_id = publ.current_user_id());
-- NOTE: we don't allow emails to be updated, instead add a new email and delete the old one.
create policy delete_own on publ.user_emails for delete using (user_id = publ.current_user_id());

create policy admin_all on publ.user_emails for all using (publ.current_user_is_admin()) with check (publ.current_user_is_admin());

grant select on publ.user_emails to :DATABASE_VISITOR;
grant insert (email) on publ.user_emails to :DATABASE_VISITOR;
-- No update
grant delete on publ.user_emails to :DATABASE_VISITOR;


-- Prevent deleting the user's last email, otherwise they can't access password reset/etc.
create or replace  function publ.tg_user_emails__prevent_delete_last_email() returns trigger as $$
begin
  if exists (
    with remaining as (
      select user_emails.user_id
      from publ.user_emails
      -- `deleted` is the user_emails row that was deleted. The variable exists because we are in a function that returns a trigger, used on a delete event
      inner join deleted
...
create trigger _500_prevent_delete_last
  after delete on publ.user_emails
  referencing old table as deleted
  for each statement
  execute procedure publ.tg_user_emails__prevent_delete_last_email();
 ...
 */
 drop table if exists priv.user_email_secrets cascade;
create table priv.user_email_secrets (
  user_email_id uuid primary key references publ.user_emails on delete cascade,
  verification_token text,
  verification_email_sent_at timestamptz,
  password_reset_email_sent_at timestamptz
);
alter table priv.user_email_secrets enable row level security;

comment on table priv.user_email_secrets is
...
comment on column priv.user_email_secrets.password_reset_email_sent_at is
  E'We store the time the last password reset was sent to this email to prevent the email getting flooded.';


create or replace function priv.tg_user_email_secrets__insert_with_user_email() returns trigger as $$
declare
...
begin
  ...
  insert into priv.user_email_secrets(user_email_id, verification_token) values(NEW.id, v_verification_token);
  return NEW;
end;
$$ language plpgsql volatile security definer set search_path to pg_catalog, public, pg_temp;
...
create trigger _500_insert_secrets
  after insert on publ.user_emails
  for each row
  execute procedure priv.tg_user_email_secrets__insert_with_user_email();

comment on function priv.tg_user_email_secrets__insert_with_user_email() is
 ...
 */
create or replace function publ.verify_email(user_email_id uuid, token text) returns boolean as $$
begin
 ...
 * `isVerified` check.
 */
create or replace function publ.tg_user_emails__verify_account_on_verified() returns trigger as $$
begin
